...
async function runSuite1() {
  await suite('Suite 1 — Security', async () => {
    ...
    const r4 = await request({ method: 'POST', path: '/run', headers: { 'Content-Type': 'application/json', 'X-YellyClaw-Token': 'wrongtoken123' } }, { prompt: 'test' });
    assert(r4.status === 403, '1.4 POST with wrong CSRF token → 403');

    // 1.5 POST with valid CSRF token but missing prompt → 400
    await fetchToken();
    const r5 = await request({ method: 'POST', path: '/run', headers: { 'Content-Type': 'application/json', 'X-YellyClaw-Token': CSRF_TOKEN } }, {});
...
async function runSuite2() {
  await suite('Suite 2 — Session lifecycle', async () => {
    ...
    const r1 = await post('/run', { prompt: 'echo hello' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r6run = await post('/run', { prompt: 'sleep 30' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    const killId = r6run.body && r6run.body.sessionId;
    await sleep(300);
    const r6kill = await post('/sessions/' + killId + '/kill', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r7 = await post('/sessions/9999/kill', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    assert(r7.status === 404, '2.7 Kill non-existent session → 404');

    // 2.8 Rerun session
    if (sessionId) {
      const r8 = await post('/sessions/' + sessionId + '/rerun', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
...
async function runSuite3() {
  await suite('Suite 3 — Self-spawn', async () => {
    ...
    const parentRes = await post('/run', { prompt: 'parent session' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r1 = await post('/sessions/' + parentId + '/spawn', { prompt: 'subtask' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    let last429 = false;
    for (let i = 0; i < 6; i++) {
      const r = await post('/sessions/' + parentId + '/spawn', { prompt: 'child ' + i }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r4 = await post('/sessions/9999/spawn', { prompt: 'orphan' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    assert(r4.status === 404, '3.4 Spawn from non-existent parent → 404');

    // 3.5 Child inherits parent agentSpec
    const parentWithAgentRes = await post('/run', { prompt: 'agent parent', agentSpec: 'test-agent' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    const agentParentId = parentWithAgentRes.body && parentWithAgentRes.body.sessionId;
    await sleep(200);
    const childRes = await post('/sessions/' + agentParentId + '/spawn', { prompt: 'child without spec' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
...
async function runSuite4() {
  await suite('Suite 4 — Scheduler', async () => {
    ...
    // 4.4 Pause schedule
    const r4 = await post('/schedules/' + scheduleId, { enabled: false }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    // 4.5 Resume schedule
    const r5 = await post('/schedules/' + scheduleId, { enabled: true }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    // 4.6 Edit schedule
    const r6 = await post('/schedules/' + scheduleId, { prompt: 'new prompt' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    // 4.7 Delete schedule
    const r7 = await post('/schedules/' + scheduleId + '?action=delete', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r8 = await post('/schedules/' + trigId + '/run', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r9 = await post('/schedules/' + trigId + '/run', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    // Trigger it
    if (onceId) {
      await post('/schedules/' + onceId + '/run', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    }
    ...
    const ids = [];
    for (let i = 0; i < 3; i++) {
      const r = await post('/schedules', { name: 'Batch ' + i, prompt: 'batch ' + i, interval: '1d' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
      if (r.body && r.body.schedule) ids.push(r.body.schedule.id);
    }
    assert(ids.length === 3, '4.13 Created 3 schedules for batch delete');
    const batchRes = await post('/schedules?action=batch-delete', { ids: [ids[0], ids[1]] }, { 'X-YellyClaw-Token': CSRF_TOKEN });
...
async function runSuite5() {
  await suite('Suite 5 — Export & history', async () => {
    ...
    const runRes = await post('/run', { prompt: 'export test session' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r4 = await post('/history/clear', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    ...
    const r6run = await post('/run', { prompt: 'lazy load test' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
...
async function runSuite7() {
  await suite('Suite 7 — Evolve Me', async () => {
    ...
    const r2 = await post('/evolve', {}, { 'X-YellyClaw-Token': CSRF_TOKEN });
    assert(r2.status === 400, '7.2 POST /evolve missing prompt → 400');

    // 7.3 POST /evolve valid
    const r3 = await post('/evolve', { prompt: 'add dark mode' }, { 'X-YellyClaw-Token': CSRF_TOKEN });
